...
func (r *courseProgramRepo) GetProgram(ctx context.Context, courseID string) (string, []domain.CourseProgramMeeting, *string, error) {
	...
	rows, err := r.pool.Query(ctx, `
		SELECT meeting_number, title, detail_text, pdf_url, pr_title, pr_description, live_class_url
	...
	for rows.Next() {
		...
		if err := rows.Scan(&m.MeetingNumber, &m.Title, &m.DetailText, &m.PdfURL, &m.PrTitle, &m.PrDescription, &m.LiveClassURL); err != nil {
			return "", nil, nil, err
		}
...
func (r *courseProgramRepo) SaveProgram(ctx context.Context, courseID string, track string, meetings []domain.CourseProgramMeeting, pretestTryoutSessionID *string) error {
	...
	for _, m := range meetings {
		if _, err := tx.Exec(ctx, `
			INSERT INTO course_meetings (course_id, meeting_number, title, detail_text, pdf_url, pr_title, pr_description, live_class_url, sort_order)
			VALUES ($1::uuid, $2, $3, $4, $5, $6, $7, $8, $2)
		`, courseID, m.MeetingNumber, m.Title, m.DetailText, m.PdfURL, m.PrTitle, m.PrDescription, m.LiveClassURL); err != nil {
			return err
		}
	}
...
func (r *courseProgramRepo) rebuildJourneyMeetings(ctx context.Context, tx pgx.Tx, courseID string, meetings []domain.CourseProgramMeeting, pretest *string) error {
	...
	if pretest != nil && *pretest != "" {
		...
		secOrder++
		if err := insertLessonTx(ctx, tx, sid, "quiz", "Pre-test", 0, nil, nil, nil, pretest); err != nil {
			return err
		}
	}
	...
	for _, m := range meetings {
		...
		if err := insertLessonTx(ctx, tx, sid, "text", lessonTitle, lo, m.DetailText, nil, nil, nil); err != nil {
			return err
		}
		lo++
		...
		if m.PdfURL != nil && strings.TrimSpace(*m.PdfURL) != "" {
			...
			if err := insertLessonTx(ctx, tx, sid, "text", "Modul PDF", lo, nil, &pdf, nil, nil); err != nil {
				return err
			}
			lo++
		}
		...
		if hasPR {
			...
			if err := insertLessonTx(ctx, tx, sid, "assignment", pt, lo, m.PrDescription, nil, nil, nil); err != nil {
				return err
			}
			lo++
		}
		...
		if m.LiveClassURL != nil && strings.TrimSpace(*m.LiveClassURL) != "" {
			...
			if err := insertLessonTx(ctx, tx, sid, "text", "Kelas live", lo, nil, nil, &live, nil); err != nil {
				return err
			}
		}
	}
...
func (r *courseProgramRepo) rebuildJourneyTryout(ctx context.Context, tx pgx.Tx, courseID string, pretest *string) error {
	...
	for rows.Next() {
		...
		secOrder++
		tid := sessionID
		if err := insertLessonTx(ctx, tx, sid, "quiz", secTitle, 0, nil, nil, nil, &tid); err != nil {
			return err
		}
	}
	return rows.Err()
...
func insertLessonTx(ctx context.Context, tx pgx.Tx, sectionID, lessonType, title string, sortOrder int, content, pdfURL, liveURL, tryoutSessionID *string) error {
	_, err := tx.Exec(ctx, `
		INSERT INTO learning_lessons (section_id, type, title, sort_order, content, pdf_url, live_class_url, tryout_session_id)
		VALUES ($1::uuid, $2::learning_lesson_type, $3, $4, $5, $6, $7, $8::uuid)
	`, sectionID, lessonType, title, sortOrder, content, pdfURL, liveURL, tryoutSessionID)
	return err
}
