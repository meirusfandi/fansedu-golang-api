...
func (r *attemptAnswerRepo) GetByAttemptAndQuestion(ctx context.Context, attemptID, questionID string) (domain.AttemptAnswer, error) {
	row := r.pool.QueryRow(ctx, `
		SELECT id, attempt_id, question_id, answer_text, selected_option, is_marked, is_correct, created_at, updated_at
		FROM attempt_answers WHERE attempt_id = $1::uuid AND question_id = $2::uuid
	`, attemptID, questionID)
	var a domain.AttemptAnswer
	err := row.Scan(&a.ID, &a.AttemptID, &a.QuestionID, &a.AnswerText, &a.SelectedOption, &a.IsMarked, &a.IsCorrect, &a.CreatedAt, &a.UpdatedAt)
	return a, err
}
...
func (r *attemptAnswerRepo) ListByAttemptID(ctx context.Context, attemptID string) ([]domain.AttemptAnswer, error) {
	...
	for rows.Next() {
		var a domain.AttemptAnswer
		if err := rows.Scan(&a.ID, &a.AttemptID, &a.QuestionID, &a.AnswerText, &a.SelectedOption, &a.IsMarked, &a.IsCorrect, &a.CreatedAt, &a.UpdatedAt); err != nil {
			return nil, err
		}
		list = append(list, a)
	}
...
func (r *attemptAnswerRepo) ListByQuestionFromSubmittedAttempts(ctx context.Context, tryoutSessionID, questionID string) ([]domain.AttemptAnswer, error) {
	rows, err := r.pool.Query(ctx, `
		SELECT aa.id, aa.attempt_id, aa.question_id, aa.answer_text, aa.selected_option, aa.is_marked, aa.is_correct, aa.created_at, aa.updated_at
