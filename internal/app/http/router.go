...
func NewRouter(deps *handlers.Deps) http.Handler {
	...
	registerV1 := func(r chi.Router) {
		...
		r.With(middleware.Auth(deps.JWTSecret)).Post("/generate-questions", handlers.GenerateQuestions(deps))
		r.With(middleware.Auth(deps.JWTSecret)).Post("/submit-answer", handlers.SubmitAnswer(deps))
		r.With(middleware.Auth(deps.JWTSecret)).Get("/analysis", handlers.GetAnalysis(deps))
		r.Get("/ranking", handlers.GetRanking(deps))
		r.Get("/questions", handlers.ListQuestions(deps))
		r.With(middleware.Auth(deps.JWTSecret)).Post("/subscription", handlers.CreateSubscription(deps))
		...
		r.Route("/trainer", func(r chi.Router) {
			...
			r.Route("/tryouts", func(r chi.Router) {
				...
				r.Post("/{tryoutId}/attempts/{attemptId}/ai-analysis", handlers.TrainerAttemptAIAnalysis(deps))
				r.Get("/{tryoutId}/attempts/{attemptId}/review", handlers.TrainerGetAttemptReview(deps))
				r.Post("/{tryoutId}/attempts/{attemptId}/review", handlers.TrainerGetAttemptReview(deps))
		...
		r.Route("/admin", func(r chi.Router) {
			...
			r.With(middleware.RequirePermission("courses.manage")).Delete("/courses/{courseId}", handlers.AdminDeleteCourse(deps))
			...
			r.Route("/tryouts", func(r chi.Router) {
				...
				r.Route("/{tryoutId}", func(r chi.Router) {
					...
					r.With(middleware.RequirePermission("tryouts.manage")).Post("/attempts/{attemptId}/ai-analysis", handlers.AdminGetAttemptAIAnalysis(deps))
					r.With(middleware.RequirePermission("tryouts.manage")).Get("/attempts/{attemptId}/review", handlers.AdminGetAttemptReview(deps))
					r.With(middleware.RequirePermission("tryouts.manage")).Post("/attempts/{attemptId}/review", handlers.AdminGetAttemptReview(deps))
